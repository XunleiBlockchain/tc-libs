...
import (
	...
	"github.com/XunleiBlockchain/tc-libs/crypto/randentropy"
...
func DecryptKey(keyjson []byte, auth string) (*Key, error) {
	...
	tt := crypto.CryptoTypeSecp256K1
	if t, ok := m["tt"].(string); ok {
		if t != "" {
			tt = t
		}
	}

	pk, err := crypto.GeneratePrivKeyFromSecret(keyBytes, tt)
	if err != nil {
		return nil, err
	}
	return &Key{
