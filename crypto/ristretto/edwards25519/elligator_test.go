...
func TestToJacobiQuarticRistretto(t *testing.T) {
	var buf [32]byte
	var feZero, feOne, fe FieldElement
	var cp, cp2 CompletedPoint
	var ep, ep2 ExtendedPoint
	...
	for i := 0; i < 1000; i++ {
		if i == 0 {
			ep = ExtendedPoint{feZero, feOne, feOne, feZero}
		} else if i == 1 {
			ep = ExtendedPoint{feOne, feZero, feOne, feZero}
		} else {
			rnd.Read(buf[:])
			fe.SetBytes(&buf)
			cp.SetRistrettoElligator2(&fe)
...
func TestRistrettoElligator2Inverse(t *testing.T) {
	var buf [32]byte
	var fe FieldElement
	var torsion [4]ExtendedPoint
	var cp, cp2 CompletedPoint
	var ep, ep2 ExtendedPoint
	var fs [8]FieldElement
