...
func TestMatcherWildcards(t *testing.T) {
	matcher := NewMatcher(testSectionSize, [][][]byte{
		{common.EmptyAddress.Bytes(), common.Address{0x01}.Bytes()}, // Default address is not a wildcard
		{common.EmptyHash.Bytes(), common.Hash{0x01}.Bytes()},       // Default hash is not a wildcard
