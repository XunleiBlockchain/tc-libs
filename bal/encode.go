...
// Encode writes the bal encoding of val to w. Note that Encode may
...
// EncodeBAL. This is true even for nil pointers, please see the
...
// pointer to a struct type always encodes as an empty bal list.
// A nil pointer to an array encodes as an empty list (or empty string
// if the array has element type byte).
//
// Struct values are encoded as an bal list of all their encoded
// public fields. Recursive struct types are supported.
//
// To encode slices and arrays, the elements are encoded as an bal
// list of the value's elements. Note that arrays and slices with
// element type uint8 or byte are always encoded as an bal string.
//
// A Go string is encoded as an bal string.
//
// An unsigned integer value is encoded as an bal string. Zero always
// encodes as an empty bal string. Encode also supports *big.Int.
//
...
func Encode(w io.Writer, val interface{}) error {
	if outer, ok := w.(*encbuf); ok {
		// Encode was called by some type's EncodeBAL.
...
func EncodeWithType(w io.Writer, val interface{}) error {
	...
	defer encbufPool.Put(eb)
	eb.reset()
	if err := eb.encodeWithPrefix(val); err != nil {
		return err
	}
	return eb.toWriter(w)
}
...
func MustEncodeToBytesWithType(val interface{}) []byte {
	bz, err := EncodeToBytesWithType(val)
	if err != nil {
		panic(err)
	}
	return bz
...
// EncodeToBytes returns the bal encoding of val.
...
// EncodeToReader returns a reader from which the bal encoding of val
...
// encbuf implements io.Writer so it can be passed it into EncodeBAL.
...
func (w *encbuf) encodeWithPrefix(val interface{}) error {
	rval := reflect.ValueOf(val)
	err := addDisfix(rval, w)
	if err != nil {
		return err
	}
	ti, err := cachedTypeInfo(rval.Type(), tags{})
...
func writeFloat(val reflect.Value, w *encbuf) error {
	f := val.Float()
	i := math.Float64bits(f)
	writeUint(reflect.ValueOf(i), w)
	return nil
}
...
func writeInt(val reflect.Value, w *encbuf) error {
	s := strconv.FormatInt(val.Int(), 16)
	if len(s) == 1 && s[0] <= 0x7f {
		// fits single byte, no string header
...
func writeBigInt(i *big.Int, w *encbuf) error {
	if cmp := i.Cmp(big0); cmp == -1 {
		return fmt.Errorf("bal: cannot encode negative *big.Int")
	} else if cmp == 0 {
		w.str = append(w.str, 0x80)
	} else {
...
func writeEncoder(val reflect.Value, w *encbuf) error {
	return val.Interface().(Encoder).EncodeBAL(w)
}
...
func writeEncoderNoPtr(val reflect.Value, w *encbuf) error {
	if !val.CanAddr() {
		...
		return fmt.Errorf("bal: game over: unadressable value of type %v, EncodeBAL is pointer method", val.Type())
	}
	return val.Addr().Interface().(Encoder).EncodeBAL(w)
}

func writeInterface(val reflect.Value, w *encbuf) error {
	if val.IsNil() {
		// Write empty list. This is consistent with the previous bal
...
func makeStructWriter(typ reflect.Type) (writer, error) {
	...
	case timeType:
		w = func(val reflect.Value, w *encbuf) error {
			...
			lh := w.list()
			writeInt(reflect.ValueOf(s), w)
			writeInt(reflect.ValueOf(ns), w)
			w.listEnd(lh)
			return nil
		}
	default:
...
//just support map[common.Address]*big.Int for encoding Account struct
func makeMapWriter(typ reflect.Type) (writer, error) {
	...
	w = func(val reflect.Value, w *encbuf) error {
		lh := w.list()
		writeInt(reflect.ValueOf(val.Len()), w)
		//keys sort
		mapKeys := make(sortableMapKey, val.Len())
		for i, krv := range val.MapKeys() {
			if !krv.CanAddr() {
				// Slice requires the value to be addressable.
				// Make it addressable by copying.
				copy := reflect.New(krv.Type()).Elem()
				copy.Set(krv)
				krv = copy
			}
			size := krv.Len()
			bz := krv.Slice(0, size).Bytes()
			mapKeys[i] = &mapKey{
				key:   bz,
				value: krv,
			}
		...
		for _, mapKey := range mapKeys {
			...
			var vrv, _, _ = derefPointers(val.MapIndex(mapKey.value))
			err := writeByteArray(mapKey.value, w)
			if err != nil {
				return err
			}
			err = writeBigIntNoPtr(vrv, w)
			if err != nil {
				return err
			}
		}
...
type sortableMapKey []*mapKey

func (sm sortableMapKey) Len() int {
	return len(sm)
}
func (sm sortableMapKey) Swap(i, j int) {
	sm[i], sm[j] = sm[j], sm[i]
}
func (sm sortableMapKey) Less(i, j int) bool {
	return bytes.Compare(sm[i].key, sm[j].key) < 0
}
