...
var (
	...
	// internal errors
	errNotInList     = errors.New("bal: call of ListEnd outside of any list")
	errNotAtEOL      = errors.New("bal: call of ListEnd not positioned at EOL")
	errUintOverflow  = errors.New("bal: uint overflow")
	errNoPointer     = errors.New("bal: interface given to Decode must be a pointer")
	errDecodeIntoNil = errors.New("bal: pointer given to Decode must not be nil")
)

// Decoder is implemented by types that require custom bal
// decoding rules or need to decode into private fields.
//
// The DecodeBAL method should read one value from the given
...
// Decode parses bal-encoded data from r and stores the result in the
...
// will be reused.
//
// To decode into a struct, Decode expects the input to be an bal
...
// the input must be an bal string. Array types decode similarly, with
// the additional restriction that the number of input elements (or
// bytes) must match the array's length.
//
// To decode into a Go string, the input must be an bal string. The
// input bytes are taken as-is and will not necessarily be valid UTF-8.
//
// To decode into an unsigned integer type, the input must also be an bal
// string. The bytes are interpreted as a big endian representation of
// the integer. If the bal string is larger than the bit size of the
...
func DecodeWithType(r io.Reader, val interface{}) error {
	// TODO: this could use a Stream from a pool.
	return NewStream(r, 0).DecodeWithPrefix(val)
}

//DecodeReader new stream with limit
func DecodeReader(r io.Reader, val interface{}, inputLimit int64) (n int64, err error) {
	// TODO: this could use a Stream from a pool.
	stream := NewStream(r, uint64(inputLimit))
	err = stream.Decode(val)
	n = int64(stream.totalSize)
	return
}
...
func DecodeReaderWithType(r io.Reader, val interface{}, inputLimit int64) (n int64, err error) {
	// TODO: this could use a Stream from a pool.
	stream := NewStream(r, uint64(inputLimit))
	err = stream.DecodeWithPrefix(val)
	n = int64(stream.totalSize)
	return
}

// DecodeBytes parses bal data from b into val.
...
func DecodeBytesWithType(b []byte, val interface{}) error {
	// TODO: this could use a Stream from a pool.
	r := bytes.NewReader(b)
	if err := NewStream(r, uint64(len(b))).DecodeWithPrefix(val); err != nil {
		return err
	}
	if r.Len() > 0 {
		return ErrMoreThanOneValue
...
func (err *decodeError) Error() string {
	...
	return fmt.Sprintf("bal: %s for %v%s", err.msg, err.typ, ctx)
}
...
func makeDecoder(typ reflect.Type, tags tags) (dec decoder, err error) {
	...
	case kind == reflect.Interface:
		return decodeCDCInterface, nil
	default:
		return nil, fmt.Errorf("bal: type %v is not bal-serializable", typ)
	}
}
...
func decodeFloat(s *Stream, val reflect.Value) error {
	var i = new(uint64)
	err := decodeUint(s, reflect.ValueOf(i).Elem())
	if err != nil {
		return err
	}
...
func makeListDecoder(typ reflect.Type, tag tags) (decoder, error) {
	...
	default:
		...
		if elemType.Kind() == reflect.Ptr {
			elemdec, err := makeOptionalPtrDecoder(elemType)
			if err != nil {
				return nil, err
			}
			dec = func(s *Stream, val reflect.Value) error {
				return decodeListSlice(s, val, elemdec)
			}
		} else {
			dec = func(s *Stream, val reflect.Value) error {
...
func makeStructDecoder(typ reflect.Type) (decoder, error) {
	...
	case timeType:
		dec = func(s *Stream, val reflect.Value) (err error) {
			...
			var sec = new(int64)
			var nsec = new(int32)
			decodeInt(s, reflect.ValueOf(sec).Elem())
			decodeInt(s, reflect.ValueOf(nsec).Elem())
...
func makeMapDecoder(typ reflect.Type) (decoder, error) {
	...
	valTyp := typ.Elem()
	switch {
	case keyTyp.Kind() == reflect.Array && isByte(keyTyp.Elem()) && 20 == keyTyp.Len():
	default:
		return nil, fmt.Errorf("bal: type %v is not bal-decode", typ)
	}
	switch {
	case valTyp.AssignableTo(reflect.PtrTo(bigInt)):
	default:
		return nil, fmt.Errorf("bal: type %v is not bal-decode", typ)
	}
	...
	dec = func(s *Stream, val reflect.Value) (err error) {
		if !val.CanAddr() {
			return fmt.Errorf("bal: val %v is not addressable", val)
		}
		...
		var len int
		err = decodeInt(s, reflect.ValueOf(&len).Elem())
		if err != nil {
			return err
		}

		var (
			mrv = reflect.MakeMapWithSize(val.Type(), len)
		)
		for len > 0 {
			keyrv := reflect.New(keyTyp).Elem()
			valrv := reflect.New(valTyp).Elem()
			err = decodeByteArray(s, keyrv)
			if err != nil {
				return err
			}
			err = decodeBigInt(s, valrv)
			if err != nil {
				return err
			}
			mrv.SetMapIndex(keyrv, valrv)
			len--
		}
		val.Set(mrv)
		return wrapStreamError(s.ListEnd(), typ)
...
func decodeInterface(s *Stream, val reflect.Value) error {
	if val.Type().NumMethod() != 0 {
		return fmt.Errorf("bal: type %v is not bal-serializable", val.Type())
	}
	kind, _, err := s.Kind()
	if err != nil {
		return err
...
func decodeDecoderNoPtr(s *Stream, val reflect.Value) error {
	return val.Addr().Interface().(Decoder).DecodeBAL(s)
}
...
func decodeDecoder(s *Stream, val reflect.Value) error {
	...
	return val.Interface().(Decoder).DecodeBAL(s)
}
...
// Bytes reads an bal string and returns its contents as a byte slice.
// If the input does not contain an bal string, the returned
...
// Uint reads an bal string of up to 8 bytes and returns its contents
// as an unsigned integer. If the input does not contain an bal string, the
...
// Bool reads an bal string of up to 1 byte and returns its contents
// as a boolean. If the input does not contain an bal string, the
// returned error will be ErrExpectedString.
func (s *Stream) Bool() (bool, error) {
	...
	case 1:
		return true, nil
	default:
		return false, fmt.Errorf("bal: invalid boolean value: %d", num)
	}
}

// List starts decoding an bal list. If the input does not contain a
...
// DecodeWithPrefix decodes a value and stores the result in the value pointed
// to by val. Please see the documentation for the Decode function
// to learn about the decoding rules.
...
func (s *Stream) readKind() (kind Kind, size uint64, err error) {
	...
	case b < 0x80:
		...
		// is its own bal encoding.
		s.byteval = b
		return Byte, 0, nil
	case b < 0xB8:
		// Otherwise, if a string is 0-55 bytes long,
		// the bal encoding consists of a single byte with value 0x80 plus the
		// length of the string followed by the string. The range of the first
		// byte is thus [0x80, 0xB7].
		return String, uint64(b - 0x80), nil
	case b < 0xC0:
		// If a string is more than 55 bytes long, the
		// bal encoding consists of a single byte with value 0xB7 plus the length
	...
	case b < 0xF8:
		...
		// bal encoding consists of a single byte with value 0xC0 plus the length
		// of the list followed by the concatenation of the bal encodings of the
		// items. The range of the first byte is thus [0xC0, 0xF7].
		return List, uint64(b - 0xC0), nil
	default:
		// If the total payload of a list is more than 55 bytes long,
		// the bal encoding consists of a single byte with value 0xF7
		// plus the length of the length of the payload in binary
		// form, followed by the length of the payload, followed by
		// the concatenation of the bal encodings of the items. The
